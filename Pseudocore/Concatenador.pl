...
### You can also specify the input dir -d dir2 \n
#########################################################################\n
###      Algorithm \n
...
##### Concatena en el hash las secuencias correspondientes (una por cada archivo)
...
my $dir2=shift;
my @keys;
print "Enter an existing gene file to get genome ids\n";
my $num=<STDIN>;
print "You have chose $num\n";
my $OpenFile="$dir2/$num";
...
opendir (DIR, $dir2) or die "Couldnt open $dir2 \n $!";  ### Abriendo el directorio
while (my $file = readdir(DIR)) { ####leyendo todos los archivos
	if ($file=~m/^\d/&&($file=~m/gb$/)){ ######## Si el nombre del archivo empieza con un dig
...
foreach my $fastaTotal (@{$reffiles}){  ## Para cada archivo con nombre numerico
	...
	my $OpenFile="$dir2/$fastaTotal"; #opening the file fastaTotal
